  ...
  <div class="container website">
    ...
    <div class="mainbox clearfix">
      ...
      <div class="search-content-main">
        ...
        <base-panel>
          <search-bar :keywords="reqParams.keywords" :showResultSize="true" :resultSize="totalCount" @searchEvent="handleSearchEvent" />
            ...
            <div class="search-event-main clearfix">
              <div class="event-search-left">
                ...
                <div class="event-table-box clearfix">
                  <base-table :list="eventList" tableHeight="480px">
                    <el-table-column prop="title" width="700px" label="网站名称">
                      <template slot-scope="scope">
                        <div class="web-container">
                          <el-image class="web-image" :src="scope.row.image" :preview-src-list="[scope.row.image]"></el-image>
                          <div class="web-info">
                            <div class="web-title">
                              <div v-if="showAllList[scope.$index].showAll1">
                                <span style="color: #fff;" v-html="highLight(scope.row.title, reqParams.keywords)"></span>
                                <span class="full-information" @click="handleExpandchange(1, scope.$index)">&nbsp;收起&nbsp;&gt;</span>
                              </div>
                              <div v-else>
                                <span style="color: #fff;" v-html="highLight(limitTextLength(scope.row.title, 460, 1), reqParams.keywords)"></span>
                                <span class="full-information" @click="handleExpandchange(1, scope.$index)" v-if="textTooLong(scope.row.title, 460, 1)">&nbsp;展开&nbsp;&gt;</span>
                              </div>
                            </div>
                            <div class="web-describe">
                              <div v-if="showAllList[scope.$index].showAll2">
                                <span v-html="highLight(scope.row.describe, reqParams.keywords)"></span>
                                <span class="full-information" @click="handleExpandchange(2, scope.$index)">&nbsp;收起&nbsp;&gt;</span>
                              </div>
                              <div v-else>
                                <span v-html="highLight(limitTextLength(scope.row.describe, 560), reqParams.keywords)"></span>
                                <span class="full-information" @click="handleExpandchange(2, scope.$index)" v-if="textTooLong(scope.row.describe, 560, 2)">&nbsp;展开&nbsp;&gt;</span>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <el-table-column prop="category" width="350px" label="类别">
                      <template slot-scope="scope">
                        <div v-if="showAllList[scope.$index].showAll3">
                          <span>{{scope.row.category}}</span>
                          <span
                            class="full-information"
                            @click="handleExpandchange(3, scope.$index)"
                          >&nbsp;收起&nbsp;&gt;</span>
                        </div>
                        <div v-else>
                          <span>{{limitTextLength(scope.row.category, 330)}}</span>
                          <span
                            class="full-information"
                            @click="handleExpandchange(3, scope.$index)"
                            v-if="textTooLong(scope.row.category, 330)"
                          >&nbsp;展开&nbsp;&gt;</span>
                        </div>
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column align="center" prop="administrator" label="管理员"></el-table-column> -->
                  ...
                  <div class="pagenation-box clearfix">
                    <base-pagination :currentPage="reqParams.currentPage" :pageSize="reqParams.pageSize" :total="totalCount"
...
import { formatDate, isEmptyObject, highLight, limitTextLength, textTooLong } from '@/utils/index.js'
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      resultData: state => state.website.resultData
    }),
    totalCount () {
    ...
    groupList () {
      if (
        ...
        !this.resultData.tagList.groupList
      ) {
        return []
      }
      return this.resultData.tagList.groupList
    },
    userList () {
      if (
        !this.resultData ||
  ...
  methods: {
    ...mapActions({ getRemoteData: 'website/getRemoteData' }),
    formatDate,
    highLight,
    limitTextLength,
    textTooLong,
    ...
    sendRequest () {
      ...
      data = Object.assign({}, data, {
        ...
        tag: this.currentIndex !== '' ? this.tagList[this.currentIndex].value : ''
      })
    ...
    handleExpandchange (type, idx) {
      this.$set(this.showAllList[idx], ['showAll' + type], !this.showAllList[idx]['showAll' + type])
    }
  },
  created () {
    if (this.isFilterSearch && (this.searchParams.startTime || this.searchParams.endTime)) {
