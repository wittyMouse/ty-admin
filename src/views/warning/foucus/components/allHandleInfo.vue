      ...
      <div class="table-list-cont" v-for="(item, index) in resultData" :key="item.id">
        <div class="tabulation-item">
          ...
          <div class="tiotabulan-text">
            <div class="width">
              <span class="tiotabulan-title" :style="hasDetail ? {cursor: 'pointer'} : ''" @click="handleDetailClick"><a>{{item.title}}</a></span>
            ...
            <p class="width clear" :title="item.content" v-html="`内容：${item.isOpen ? item.content : item.digest}`"></p>
            <div class="tabulation-subfield">
              <span>来自：{{item.source}}</span>
              <!-- <span class="text-center" v-show="item.url">链接地址：{{item.url}}</span> -->
