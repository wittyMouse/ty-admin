            ...
            <div class="search-event-main clearfix">
              <div class="event-search-left">
                ...
                <div class="event-table-box clearfix">
                  <base-table :list="users" tableHeight="480px" @row-dblclick="handleRowClick">
                    <el-table-column width="500px" prop="title" label="标题和内容">
                      <template slot-scope="scope">
                        ...
                        <div v-else>
                          <span>{{limitTextLength(scope.row.title, 480)}}</span>
                          <span
                            class="full-information"
                            @click.stop="handleExpandchange(1, scope.$index)"
                            v-if="textTooLong(scope.row.title, 480)"
                          >&nbsp;展开&nbsp;&gt;</span>
                        </div>
                      </template>
                    </el-table-column>
                    <el-table-column width="350px" prop="category" label="地点">
                      <template slot-scope="scope">
                    ...
                    <el-table-column width="200px" prop="username" label="发布者">
                      <template slot-scope="scope">
                        <div v-if="showAllList[scope.$index].showAll3">
                          <span v-html="highLight(scope.row.username, reqParams.keywords)"></span>
                          <span
                            class="full-information"
                        ...
                        <div v-else>
                          <span
                            v-html="highLight(limitTextLength(scope.row.username, 180), reqParams.keywords)"
                          ></span>
                          <span
                            class="full-information"
                            @click.stop="handleExpandchange(3, scope.$index)"
                            v-if="textTooLong(scope.row.username, 180)"
                          >&nbsp;展开&nbsp;&gt;</span>
                        </div>
                      </template>
                    </el-table-column>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      resultData: state => state.people.resultData
    }),
    totalCount () {
    ...
    users () {
      if (!this.resultData || !this.resultData.users) {
        return []
      }
      const list = []
      this.resultData.users.map(() => {
        list.push({ showAll1: false, showAll2: false, showAll3: false })
      })
      this.showAllList = list
      return this.resultData.users
    },
    groupList () {
      if (
        !this.resultData ||
  ...
  methods: {
    ...mapActions({ getRemoteData: 'people/getRemoteData' }),
    formatDate,
    highLight,
    limitTextLength,
    textTooLong,
    ...
    // 点击行
    handleRowClick (row) {
      this.$router.push({ name: 'websitePage', query: { type: 'people', id: row.id } })
    },
    // 当前页改变
